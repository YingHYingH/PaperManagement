							...
							<div class="am-btn-group am-btn-group-xs">
								...
								<script type="text/javascript">
									...
									function addToMyPage(paperId,userId){
								        ...
								        //如果设置数据传送方式为post，则必须设置请求头信息
								        req.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
										var value = "paperId=" + paperId+"&userId="+userId;
										req.send(value);
										location.reload();
									}
								</script>
				...
				<div class="am-g">
					<div class="am-u-sm-12">
							<table
								...
								<%
									String followed = new UserDao().getUserById(user.getId()).getFollowed();
									ResultSet rs = null;
									if(followed.length()!=0){
										String[] followedArr = followed.substring(0, followed.length()-1).split(",");
										String inSql ="(";
										for(int i=0;i<followedArr.length;i++){
											inSql += "'"+followedArr[i]+"',";
										}
										...
										String sql = "select paper.* from paper join user on paper.user_id = user.id where user.username in "+inSql;
