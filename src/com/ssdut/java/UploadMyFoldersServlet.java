...
public class UploadMyFoldersServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
			...
			while (i.hasNext()) {
				...
				if (!fm.isFormField()) {
					...
					try {
						...
						message = "文件上传成功！";
						boolean res = addPaperDao.getInstance().addPaper(paper);
						//增加folder与paper对应表数据
						Paper paperAlreadySave = new PaperDao().getPaperByurl(fileNewPath);
						new FolderPaperDao().addFolderPaper(Integer.valueOf(folderId), paperAlreadySave.getId());
		...
		String address = "addPaper5.jsp?first=url";
